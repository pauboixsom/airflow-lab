...
class FiveTimetable(Timetable):
    ...
    def infer_manual_data_interval(self, run_after: DateTime) -> DataInterval: #al tuto i al videu està malament però ho arregla el mr taiwan https://github.com/apache/airflow/pull/19735
        ...
        elif weekday ==0: #interval de dissabte, si es fa el trigger dimarts
    ...
    def next_dagrun_info(
        ...
        *,
        ...
        if last_automated_data_interval is not None:  # There was a previous run on the regular schedule.
            ...
            if 0 <= last_start_weekday < 4:  # Last run on Tuesday through Friday -- next is tomorrow.
                delta = timedelta(days=1)
                if last_start_weekday == 3: #si l'ultim es divendres el següent és dissabte i els dissabtes són de 3 dies
            ...
            next_start = DateTime.combine((last_start + delta).date(), Time(5,0,0)).replace(tzinfo=UTC)
            ...
            if not restriction.catchup:
                ...
                next_start = max(next_start, DateTime.combine(Date.today(), Time(5,0,0)).replace(tzinfo=UTC))
            elif next_start.time() > Time(5,0,0):
                ...
                next_start = DateTime.combine(next_day, Time(5,0,0)).replace(tzinfo=UTC)
                if next_start.weekday in (4):
                    next_end=timedelta(days=3)
                else:
                    next_end=timedelta(days=1)
            #a sota no hauria d'entrar mai?
            next_start_weekday = next_start.weekday()
            if next_start_weekday in (5, 6):  # If next start is in Sunday, monday, go to next tuesday. QUE AQUEST EN REALITAT ÉS DE DISSABTE A LES 00 AL MONDAY A LES 24:00
...
class FiveTimetablePlugin(AirflowPlugin):
    name = "five_timetable_plugin"
    timetables = [FiveTimetable]